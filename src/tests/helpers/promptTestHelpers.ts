...
export interface BestPractice {
  name: 'hyper-specific' | 'character-consistency' | 'context-intent' | 
        'iterate-refine' | 'semantic-negatives' | 'aspect-ratio' | 'camera-control'
  keywords: string[]
  patterns: RegExp[]
  requirements: string[]
}
...
export function comparePromptStructure(
  ...
  const technicalTerms = [
    'wide-angle', 'macro', '85mm', 'portrait lens', 'Dutch angle',
    'low-angle', 'composition', 'lighting', 'depth of field',
    'ornate', 'etched', 'detailed', 'specific', 'high-end', 'minimalist'
  ]
  ...
  const structureScore = calculateStructureScore(
    wordCountIncrease, technicalTermsAdded, enhancedElements.length
  )
...
export function extractPOMLElements(prompt: string): POMLElements {
  ...
  if (examplesMatch) {
    elements.examples = examplesMatch[1].split('\n').map(ex => ex.trim()).filter(Boolean)
  ...
  if (constraintsMatch) {
    elements.constraints = constraintsMatch[1].split('\n').map(c => c.trim()).filter(Boolean)
 ...
 */
export function assertNegativeToPositiveConversion(
  original: string, 
  converted: string
): void {
...
function detectMissingElements(original: string, structured: string): string[] {
...
function validateSinglePractice(prompt: string, practice: BestPractice): ValidationResult {
  ...
  // Check patterns
  const patternMatches = practice.patterns.filter(pattern => 
    pattern.test(prompt)
  )
